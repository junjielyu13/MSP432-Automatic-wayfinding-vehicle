 ...
 *
 * Encontrarás en:                          Habrá dos parte de código. Y un parte de ella está comentada
 *                  1 -- config_RGB_LEDS
 *                  2 -- TA0_0_IRQHandler
 *                  3 -- TA1_0_IRQHandler
 *
 * Esto se debe a que las luces RGB de boosterpack de nuestro robot en la clase de laboratorio están rotas
 * y el RGB rojo no es brillante (Robot No. 01)
 *
 * Entonces usamos placa msp432 para probar en todo momento, Encontrará que el código comentado a continuación está dedicado a placa msp432.
 *
 *
 * Sin embargo, para enviar la practica, también escribimos código específicamente para boosterpack, que no se comenta a continuación.
...
void PORT4_IRQHandler(void)
{
    ...
    P4IE &= ~(BIT1 + BIT5 + BIT7 ); //interrupciones del boton S2 en port 3 desactivadas
    ...
    P4IE |= (BIT1 + BIT5 + BIT7 );   //interrupciones S2 en port 3 reactivadas
}
...
void PORT5_IRQHandler(void)
{
    ...
    P5IE &= ~(BIT1 + BIT4 + BIT5 ); //interrupciones del boton S2 en port 3 desactivadas
    ...
    P5IE |= (BIT1 + BIT4 + BIT5 );   //interrupciones S2 en port 3 reactivadas
}
